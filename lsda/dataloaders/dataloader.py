    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            if self.args.dataset == 'kitti':
                ...
                # depth_file = os.path.join(sample_path.split()[0].split('/')[0], sample_path.split()[1])   # 源代码
            ...
            # image_path = os.path.join(data_path, "./" + sample_path.split()[0])   # 源代码
            image_path = os.path.join(data_path, sample_path.split()[0])
            image = np.asarray(Image.open(image_path), dtype=np.float32) / 255.0
            # 加噪声
            # noise = np.random.normal(0, 0.1, image.shape)   # 均值0，标准差0.1
            # out = image + noise  # 将噪声和原始图像进行相加得到加噪后的图像
            # image = np.clip(out, 0.0, 1.0)

            if self.mode == 'online_eval':
                gt_path = self.args.gt_path_eval
                # depth_path = os.path.join(gt_path, "./" + sample_path.split()[1]) # 源代码
                depth_path = os.path.join(gt_path, sample_path.split()[1])
                if self.args.dataset == 'kitti':
                    # depth_path = os.path.join(gt_path, sample_path.split()[0].split('/')[0], sample_path.split()[1])  # 源代码
                    pass
